...
namespace ThiTracNghiemWindows
{
    public class ExcelExportPro
    {
        ...
        public const string T_DanhmucHocVien = "DanhMucHV";
        public const string T_DoanhThu = "DT";
        public const string T_TKB = "TKB";
        public const string T_LichDay = "LICHDAY";
        public const string T_DIEM = "DIEM";
        public const string T_BC_DIEMDANH = "BCDIEMDANH";
        public const string T_KETQUA = "KETQUA";
        public const string T_DanhMucGV = "DanhMucGV";
        public const string T_DanhMucLH = "DanhMucLH";
        public const string T_DanhMucMH = "DanhMucMH";
        public const string T_DanhMucNV = "DanhMucNV";
        public const string T_DMDIEM = "DMDIEM";
        #endregion
        ...
        private void BuildReplacerCurrentDate(ref Dictionary<string, string> pReplacer)
        {
            if (pReplacer != null)
            {
              ...
              //  pReplacer.Add("%TongSo",pReplacer.);
            }
        }
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
            ...
            if (!Directory.Exists(Application.StartupPath + Constants.FOLDER_TEMPLATES))
            {
                Directory.CreateDirectory(Application.StartupPath + Constants.FOLDER_TEMPLATES);
            }
            //Declare path App/Templates/Excels
              string path =Application.StartupPath + Constants.FOLDER_TEMPLATES   + Constants.CHAR_FLASH;
            ...
            switch (viewName)
            {
                ...
                case T_DanhMucKhoa:
                    ...
                    break;
                case T_DanhmucHocVien:
                    path+="DMHOCVIEN.xlsx";
                    arrByte = File.ReadAllBytes(path).ToArray();
                    break;
                case T_DoanhThu:
                    path += "DOANHTHU.xlsx";
                    arrByte = File.ReadAllBytes(path).ToArray();
                ...
                case T_DIEM:
                    ...
                    break;
                case T_BC_DIEMDANH:
                    path += "DMDIEMDANH.xlsx";
                    arrByte = File.ReadAllBytes(path).ToArray();
                    break;
                case T_DanhMucGV:
                    path += "DMGIANGVIEN.xlsx";
                    arrByte = File.ReadAllBytes(path).ToArray();
                    break;
                case T_DanhMucLH:
                    path += "DMLOPHOC.xlsx";
        ...
        public bool ExportHocVien(List<object> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            return OutSimpleReport(dataSource, replacer, T_DanhmucHocVien, isPrintPreview, ref fileName);
        }
        public bool ExportTKB(List<object> dataSource, ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            return OutSimpleReport(dataSource, replacer, T_TKB, isPrintPreview, ref fileName);
        }
         public bool ExportLichDay(List<object> dataSource, ref string fileName, bool isPrintPreview,string giangv)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            string ngay = getNgayCurrent();
            replacer.Add("%NgayThangNam", ngay);
            replacer.Add("%gv", giangv);
            return OutSimpleReport(dataSource, replacer, T_LichDay, isPrintPreview, ref fileName);
        }
        
        public bool ExportDoanhThu(List<object> dataSource, ref string fileName, bool isPrintPreview,string tongso, string ngaytrc,string ngaysau)
        {
            // Check if data is null
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
            ...
            string ngay = getNgayCurrent();
            replacer.Add("%NgayThangNam", ngay);
            replacer.Add("%tong", tongso);
            replacer.Add("%ngaytruoc", ngaytrc);
            replacer.Add("%ngaysau", ngaysau);
            return OutSimpleReport(dataSource, replacer, T_DoanhThu, isPrintPreview, ref fileName);
        }
        #endregion
        public string getNgayCurrent()
        {
            DateTime currentDate = DateTime.Now;
            string ngay = "Ngày " + currentDate.Day + " tháng " + currentDate.Month + " năm " + currentDate.Year;
            return ngay;
        }
        public bool ExportDiem(List<object> dataSource, ref string path, bool v, string tengv, string tenkhoa,string tenlh)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            string ngay = getNgayCurrent();
            replacer.Add("%NgayThangNam", ngay);
            replacer.Add("%gv", tengv);
            replacer.Add("%khoa", tenkhoa);
            replacer.Add("%lop", tenlh);
            return OutSimpleReport(dataSource, replacer, T_DIEM, v, ref path);
        }
        public bool ExportDMDiem(List<object> dataSource, ref string path, bool v)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            return OutSimpleReport(dataSource, replacer, T_DMDIEM, v, ref path);
        }


        public bool ExportDMGiangVien(List<object> dataSource, ref string path, bool v)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            return OutSimpleReport(dataSource, replacer, T_DanhMucGV, v, ref path);
        }
        public bool ExportDMNhanVien(List<object> dataSource, ref string path, bool v)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            return OutSimpleReport(dataSource, replacer, T_DanhMucNV, v, ref path);
        }

        public bool ExportDMLopHoc(List<object> dataSource, ref string path, bool v)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            return OutSimpleReport(dataSource, replacer, T_DanhMucLH, v, ref path);
        }
        public bool ExportDMMonHoc(List<object> dataSource, ref string path, bool v)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            return OutSimpleReport(dataSource, replacer, T_DanhMucMH, v, ref path);
        }

        public bool ExportBC_DiemDanh(List<object> dataSource, ref string path, bool v, string malh, string makh)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            replacer.Add("%makhoa", makh);
            replacer.Add("%malop", malh);
            return OutSimpleReport(dataSource, replacer, T_BC_DIEMDANH, v, ref path);
        }
        public bool ExportKetQua(List<object> dataSource, ref string path, bool v, int dau, int rot)
        {
            if (dataSource == null || (dataSource != null && dataSource.Count == 0))
            {
                return false;
            ...
            Dictionary<string, string> replacer = new Dictionary<string, string>();
            string ngay = getNgayCurrent();
            replacer.Add("%ngaylap", ngay);

            replacer.Add("%dau", dau.ToString());
            replacer.Add("%rot", rot.ToString());
            return OutSimpleReport(dataSource, replacer, T_KETQUA, v, ref path);
        }
    }
}
